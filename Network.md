...
* Socket：
  ...
  - write和read行为的区别：
   ...
   - sched_yield()函数：
      - 这个函数可以让另一个级别高于或等于当前线程的线程先运行，如果没有符合条件的线程，那么这个函数将会立刻返回然后继续执行当前线程的程序。
  - 为什么roundtrip中客户端需要两个线程来完成？
      - 两个线程一个用于收数据，另一个用于发数据，因为UDP不可靠，你不能发一个再等着收一个，发过去包可能会丢掉，发回来的包也可能会丢掉。
